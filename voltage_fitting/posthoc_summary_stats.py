...
def spike_times(trace_spikes: jnp.ndarray, dt=0.025, max_spikes=2):
    """Returns the first `max_spikes` spike times, padded with 10000.0 if not enough spikes."""
    spike_inds = jnp.where(trace_spikes, size=max_spikes, fill_value=-1)[0]  # shape: (max_spikes,)
    spike_times = jnp.where(spike_inds >= 0, spike_inds * dt, 10000.0)
    return spike_times 
