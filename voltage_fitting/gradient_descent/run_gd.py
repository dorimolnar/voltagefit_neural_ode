...
def main(
    ...
    t, x_o, i_amp = get_experimental_data(setup, t_max)
    simulate = setup_simulator(cell, t_max, all_setup["v_init"], i_amp)
    ...
    def predict(params):
        ...
        return simulate(params_0)[0] #(number of recordings (1), number of time points) --> (number of time points,)
    ...
    logger.info(f"opt_params.shape {opt_params.shape}") #(n_particles, lowers.shape)
    ...
    logger.info(f"10 best losses: {top_10_losses}")

    # Save.
    filename = f"Multi_250312_{setup}_{int(t_max)}_{lr}_{momentum}_{steps}_{cost_fn_power}_{seed}"
