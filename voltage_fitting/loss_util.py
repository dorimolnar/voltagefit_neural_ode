...
from voltage_fitting.posthoc_summary_stats import find_spikes
...
#@jax.jit
def get_scaled_loss(x_o, x, cost_fn_power, scale, penalty_rate = 1.0, **kwargs):
    gamma = 0.1
    ...
    def penalty_fn(rows, cols):
        return jnp.abs((rows - cols) / x_o_red.shape[0]) * penalty_rate
    ...
    # @jax.jit
    # def scaled_loss(x, scale=0.05):
    #     # x, mask = predict(params)
    #     # x = predict(params)
